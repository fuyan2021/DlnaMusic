...
public class CharsetDetector {
	...
	private static String[] sCharsets;
	...
	private static String[] getCharsets() {
		if (sCharsets == null) {
			sCharsets = new String[TOTAL_ENCODINGS];
			sCharsets[GB2312] = "GB2312";
			sCharsets[GBK] = "GBK";
			sCharsets[HZ] = "HZ";
			sCharsets[BIG5] = "BIG5";
			sCharsets[EUC_TW] = "CNS11643";
			sCharsets[ISO_2022_CN] = "ISO2022CN";
			sCharsets[UTF8] = "UTF8";
			sCharsets[UNICODE] = "Unicode";
			sCharsets[ASCII] = "ASCII";
		}
		return sCharsets;
	}
	...
	public static String getCharset(InputStream inputStream) {
		CharsetDetector detector = new CharsetDetector();
		int code = detector.detectEncoding(inputStream, 8192);
		return detector.getDecodeString(code);
	}

	public static String getDecodeCharset(String str) {
		String encodeCharset = getEncodeCharset(str);
		byte[] bytes;
		try {
			bytes = str.getBytes(encodeCharset);
		} catch (Exception e) {
			bytes = str.getBytes();
		}
		return getDecodeCharset(bytes);
	}

	public static String getDecodeCharset(byte[] bytes) {
		CharsetDetector detector = new CharsetDetector();
		int code = detector.detectEncoding(bytes);
		return detector.getDecodeString(code);
	}

	public static String getEncodeCharset(String str) {
		String[] charsets = {
			"GB2312",
			"ISO-8859-1",
			"UTF-8",
		...
		for (String charset : charsets) {
			if (isEncodeCharset(str, charset)) {
				return charset;
			}
		}
		return "";
	...
	private static boolean isEncodeCharset(String str, String charset) {
		try {
	 ...
	 * enumeration (GB2312, HZ, BIG5, EUC_TW, ASCII, or OTHER) Description: This function looks at
	 * the URL contents and assigns it a probability score for each encoding type. The encoding type
	...
	public int detectEncoding(URL url) {
		...
		try {
			...
			while ((bytesread = inputStream.read(rawtext, byteoffset, rawtext.length - byteoffset)) > 0) {
	 ...
	 * enumeration (GB2312, HZ, BIG5, EUC_TW, ASCII, or OTHER) Description: This function looks at
	 * the file and assigns it a probability score for each encoding type. The encoding type with
	...
	public int detectEncoding(File file, int length) {
		...
		int size = (int) Math.min(length, file.length());
		rawtext = new byte[size];
		try {
	...
	public int detectEncoding(InputStream inputStream, int length) {
		try {
			byte[] rawtext;

			int size = (int) Math.min(length, 1024 * 4);
			rawtext = new byte[size];
	 ...
	 * @return 解码格式
	 */
	public String getDecodeString(int detectEncoding) {
		return getCharsets()[detectEncoding];
	}
	 ...
	 * looks at the byte array and assigns it a probability score for each encoding type. The
	 * encoding type with the highest probability is returned.
	 */

	public int detectEncoding(byte[] bytes) {
		int[] scores;
		...
		try {
			...
			// Assign Scores
			scores[GB2312] = gb2312_probability(bytes);
			scores[GBK] = gbk_probability(bytes);
			scores[HZ] = hz_probability(bytes);
			scores[BIG5] = big5_probability(bytes);
			scores[EUC_TW] = euc_tw_probability(bytes);
			scores[ISO_2022_CN] = iso_2022_cn_probability(bytes);
			scores[UTF8] = utf8_probability(bytes);
			scores[UNICODE] = utf16_probability(bytes);
			scores[ASCII] = ascii_probability(bytes);
	...
	/*
	 * Function: gb2312_probability Argument: pointer to byte array Returns : number from 0 to 100
	 * representing probability text in array uses GB-2312 encoding
	 */
	...
	/*
	 * Function: hz_probability Argument: byte array Returns : number from 0 to 100 representing
	 * probability text in array uses HZ encoding
	 */
	...
	/*
	 * Function: euc_tw_probability Argument: byte array Returns : number from 0 to 100 representing
	 * probability text in array uses EUC-TW (CNS 11643) encoding
	 */
	...
	/*
	 * Function: iso_2022_cn_probability Argument: byte array Returns : number from 0 to 100
	 * representing probability text in array uses ISO 2022-CN encoding WORKS FOR BASIC CASES, BUT
	...
	/*
	 * Function: utf8_probability Argument: byte array Returns : number from 0 to 100 representing
	 * probability text in array uses UTF-8 encoding of Unicode
	 */
	...
	/*
	 * Function: utf16_probability Argument: byte array Returns : number from 0 to 100 representing
	 * probability text in array uses UTF-16 encoding of Unicode, guess based on BOM // NOT VERY
	 * GENERAL, NEEDS MUCH MORE WORK
	 */
	...
	int utf16_probability(byte[] rawtext) {
		...
		/*
		 * // Check to see if characters fit into acceptable ranges rawtextlen = rawtext.length; for
		 * (i = 0; i < rawtextlen; i++) { if ((rawtext[i] & (byte)0x7F) == rawtext[i]) { // One byte
		 * goodbytes += 1; asciibytes++; } else if ((rawtext[i] & (byte)0xDF) == rawtext[i]) { //
		 * Two bytes if (i+1 < rawtextlen && (rawtext[i+1] & (byte)0xBF) == rawtext[i+1]) {
		 * goodbytes += 2; i++; } } else if ((rawtext[i] & (byte)0xEF) == rawtext[i]) { // Three
		 * bytes if (i+2 < rawtextlen && (rawtext[i+1] & (byte)0xBF) == rawtext[i+1] &&
		 * (rawtext[i+2] & (byte)0xBF) == rawtext[i+2]) { goodbytes += 3; i+=2; } } }
		 *
		 * score = (int)(100 * ((float)goodbytes/(float)rawtext.length));
		 *
		 * // An all ASCII file is also a good UTF8 file, but I'd rather it // get identified as
		 * ASCII. Can delete following 3 lines otherwise if (goodbytes == asciibytes) { score = 0; }
		 *
		 * // If not above 90, reduce to zero to prevent coincidental matches if (score > 90) {
	...
	/*
	 * Function: ascii_probability Argument: byte array Returns : number from 0 to 100 representing
	 * probability text in array uses all ASCII Description: Sees if array has any characters not in
